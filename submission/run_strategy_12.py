...
clf.fit(data_train_copy.as_matrix()[:,:-1])
outliers = clf.predict(data_train_copy.as_matrix()[:,:-1])
...
dict_kmeans_model = fit_cluster(data_train_copy,ks=[2, 10, 20, 50, 80, 100, 200, 300],s_perc = 1)
...
for k in [2, 10, 20, 50, 80, 100, 200, 300]:
...
matrix_data = np.hstack([matrix_data,matrix_cluster_kmeans,outliers.reshape(-1,1)])
...
outliers = clf.predict(matrix_test)

matrix_test = np.hstack([matrix_test,matrix_cluster_kmeans,outliers.reshape(-1,1)])
...
result.to_csv("sub_strategy_12.csv", index=False)
